...
static int TableGetMaxDisplayOrderAllowed(ImGuiTable *table, int src_order,
  ...
  // TableSetupScrollFreeze() enforce a display order range for frozen columns.
  // Reordering across the frozen column barrier is illegal and will be undone.
...
void ImGui::EndTable() {
  ...
  if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
    ...
    // visibility/sizes to generally save a frame of feedback? See broken test
    // in 'table_reported_size_outer'
...
void ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags,
  ...
  IM_ASSERT_USER_ERROR_RET(
      table->IsLayoutLocked == false,
      "TableSetupColumn(): need to call before first row!"); // Table layout is
                                                             // locked when
                                                             // submitting a row
                                                             // or when calling
                                                             // BeginMultiSelect()
  ...
  if (table->Flags & ImGuiTableFlags_BordersOuter) {
    ...
    if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
      ...
      inner_drawlist->AddLineV(outer_border.Min.x, outer_border.Min.y,
                               outer_border.Max.y, outer_col, border_size);
      ...
      inner_drawlist->AddLineH(outer_border.Min.x, outer_border.Max.x,
                               outer_border.Min.y, outer_col, border_size);
      inner_drawlist->AddLineH(outer_border.Min.x, outer_border.Max.x,
                               outer_border.Max.y, outer_col, border_size);
    }
  }
...
void ImGui::TableHeader(const char *label) {
  ...
  if (held && (table->Flags & ImGuiTableFlags_Reorderable) &&
    ...
    // - The other constraints are enforced by TableQueueSetColumnDisplayOrder()
    // which might early out.
    table->InstanceInteracted = table->InstanceCurrent;
...
static bool MenuItemForColumnReorder(const char *label, bool selected,
  ...
  bool ret =
      ...
      !has_been_moved; // Can't use IsMouseDragging(0) as button is released
                       // already.
...
void ImGui::TableDrawDefaultContextMenu(
  ...
  const int context_column_n = (table->ContextPopupColumn >= 0 &&
  ...
  if (flags_for_section_to_display & ImGuiTableFlags_Hideable) {
    ...
    // While reordering: we calculate min/max allowed range once here so we can
    // avoid a O(N log N) in the loop (because the query itself does a sweep
    // scan). This assume that reordering constraints output a single range,
    // otherwise would need to either call TableGetMaxDisplayOrderAllowed() for
    // each item below, or cache this once per frame into columns.
    ...
    const int reorder_max_order =
        is_reordering ? TableGetMaxDisplayOrderAllowed(table, reorder_src_order,
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
      ...
      if (IsItemActive() && IsMouseDragging(0) &&
        ...
        g.ActiveIdHasBeenEditedBefore =
            true; // Disable toggle in MenuItemForColumnReorder() + start
                  // dimming to display allowed reorder targets.
        table->ReorderColumn = (ImGuiTableColumnIdx)column_n;
        if (!IsItemHovered()) {
          int reorder_dir =
              (g.IO.MousePos.y <
