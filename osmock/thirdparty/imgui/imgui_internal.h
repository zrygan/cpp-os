...
#define IM_TRUNC(_VAL)                                                         \
  ((float)(int)(_VAL)) // Positive values only! ImTrunc() is not inlined in MSVC
                       // debug builds
...
enum ImDrawTextFlags_ {
  ...
  ImDrawTextFlags_CpuFineClip = 1
                                << 0, // Must be == 1/true for legacy with 'bool
                                      // cpu_fine_clip' arg to RenderText()
...
enum ImGuiButtonFlagsPrivate_ {
  ...
  ImGuiButtonFlags_PressedOnRelease =
      1 << 7, // return true on release (default requires click+release). Prior
              // to 2026/03/20 this implied ImGuiButtonFlags_NoHoldingActiveId
              // but they are separate now.
...
enum ImGuiWindowBgClickFlags_ {
  ...
  ImGuiWindowBgClickFlags_Move =
      1 << 0, // Click on bg/void + drag to move window. Cleared by default when
              // using io.ConfigWindowsMoveFromTitleBarOnly.
};
...
enum ImGuiNavMoveFlags_ {
  ...
  ImGuiNavMoveFlags_ScrollToEdgeY =
      1 << 6, // Force scrolling to min/max (used by Home/End) // FIXME-NAV: Aim
              // to remove or reword as ImGuiScrollFlags
...
struct ImGuiViewportP : public ImGuiViewport {
  float
      BgFgDrawListsLastTimeActive[2]; // Last frame number the background (0)
                                      // and foreground (1) draw lists were used
...
struct ImGuiContext {
  ...
  float FontSize; // Currently bound font size == line height (== FontSizeBase +
                  ...
                  // function).
  float FontSizeBase; // Font size before scaling == style.FontSizeBase == value
                      // passed to PushFont() when specified.
  ...
  float WindowsBorderHoverPadding; // Padding around resizable windows for which
  ...
  ImRect
      NavScoringNoClipRect; // Some nav operations (such as PageUp/PageDown)
                            ...
                            // always keep submitted. Unset/invalid if inverted.
  ...
  // polled when no window are focused. It also doesn't work e.g. Web platforms.
  ...
  ImGuiKeyChord ConfigNavWindowingKeyNext; // = ImGuiMod_Ctrl | ImGuiKey_Tab (or
                                           // ImGuiMod_Super | ImGuiKey_Tab on
                                           // OS X). Set to 0 to disable. For
  ...
  ImGuiID
      InputTextReactivateId; // ID of InputText to reactivate on next frame (for
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImU32 TreeRecordsClippedNodesY2Mask; // Store whether we should keep recording
...
struct IMGUI_API ImGuiWindow {
  ...
  ImU8
      ScrollbarXStabilizeToggledHistory; // Used to stabilize scrollbar
                                         // visibility in case of feedback loops
...
#define IMGUI_TABLE_MAX_COLUMNS                                                \
  512 // Arbitrary "safety" maximum, may be lifted in the future if needed. Must
      // fit in ImGuiTableColumnIdx/ImGuiTableDrawChannelIdx.
...
struct IMGUI_API ImGuiTable {
  ...
  ImGuiTableColumnIdx LastHeldHeaderColumn; // Index of column header being held
                                            // from previous frame.
  ImGuiTableColumnIdx
...
struct ImFontAtlasRectEntry {
  int TargetIndex : 20; // When Used: ImFontAtlasRectId -> into Rects[]. When
                        ...
                        // consume free-list.
  unsigned int Generation : 10; // Increased each time the entry is reused for a
                                // new rectangle.
...
struct ImFontAtlasBuilder {
  ...
  ImVec2i MaxRectSize; // Largest rectangle to pack (de-facto used as a "minimum
                       // texture size")
