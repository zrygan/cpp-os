...
static ConfigStruct *fromFileAt(const std::string &path) {
    ...
    while (getline(f, line)) {
        ...
        std::string key;
        iss >> key;
        if      (key == "num-cpu")           iss >> cs->num_cpu;
        else if (key == "scheduler")         iss >> cs->scheduler;
        else if (key == "quantum-cycles")    iss >> cs->quantum_cycles;
        else if (key == "batch-process-freq")iss >> cs->batch_process_freq;
        else if (key == "min-ins")           iss >> cs->min_ins;
        else if (key == "max-ins")           iss >> cs->max_ins;
        else if (key == "delay-per-exec")    iss >> cs->delay_per_exec;
    }
    return cs;
}
...
static void validateOrDie(ConfigStruct *cs, const char *path = "") {
    ...
    if (cs->scheduler != "fcfs" && cs->scheduler != "rr") {
        std::cerr << "[prosched] 'scheduler' missing or invalid (must be 'fcfs' or 'rr')\n";
        exit(1);
    }
    if (cs->quantum_cycles < 1) {
        std::cerr << "[prosched] 'quantum-cycles' missing or invalid (must be >= 1)\n";
        exit(1);
    }
    if (cs->batch_process_freq < 1) {
        std::cerr << "[prosched] 'batch-process-freq' missing or invalid (must be >= 1)\n";
        exit(1);
    }
    if (cs->min_ins < 1) {
...
namespace ConfigValidation {
    ...
    TEST(ConfigValidation, MissingNumCpuExits) {
        writeTestConfig(
            ...
            "min-ins 100\nmax-ins 200\ndelay-per-exec 0\n");
        EXPECT_EXIT(
            { ConfigStruct *cs = fromFileAt(TEMP_CFG); validateOrDie(cs); },
            ::testing::ExitedWithCode(1), ".*"
        );
        remove(TEMP_CFG);
    }
