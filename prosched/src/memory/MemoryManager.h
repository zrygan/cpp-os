...
public:
  ...
  /**
   * @brief Constructs a MemoryManager with the specified total memory and
   * process size.
   *
  ...
  /**
   * @brief Checks if memory is currently allocated for a process with the given
   * PID.
   *
  ...
  /**
   * @brief Returns an ASCII snapshot of the memory state at the given
   * timestamp.
   *
  ...
  std::string GetAsciiSnapshot(long timestamp, int quantumCycle) const {
    ...
    for (const auto &block : blocks) {
      ...
      if (block.isFree) {
        ...
        ss << "PID: " << block.pid << " (" << (block.end - block.start)
           << " bytes)\n";
      }
    }
    return ss.str();
   ...
   *
   */
  void SaveLogsToFileMem(int quantumNum, const std::string &timestamp,
                         int processCount) {
    std::filesystem::create_directory("memory_stamps");
    std::string filename =
        "memory_stamps/memory_stamp_" + std::to_string(quantumNum) + ".txt";
    ...
    if (outFile.is_open()) {
      ...
      outFile << "Total external fragmentation in KB: "
              << GetExternalFragmentation() / static_cast<std::size_t>(1024)
              << "\n";
      outFile << "\n";
      ...
      std::cerr << "Error: Could not make log file for memory_stamp_"
                << std::to_string(quantumNum) << "\n";
    }
  }
   ...
   */
  void RemoveFromBackingStore(int pid) { backingStore.erase(pid); }
