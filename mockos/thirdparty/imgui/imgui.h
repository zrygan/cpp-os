...
enum ImGuiItemFlags_ {
  ...
  ImGuiItemFlags_Disabled =
      1 << 6, // false    // [Internal] Disable interactions. DOES NOT affect
              // visuals. This is used by BeginDisabled()/EndDisabled() and only
              // provided here so you can read back via GetItemFlags().
};
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_CtrlEnterForNewLine =
      1 << 8, // In multi-line mode: validate with Enter, add new line with
              ...
              // line with Enter). Note that Shift+Enter always enter a new line
              // either way.
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_AllowOverlap =
      1 << 2, // Hit testing will allow subsequent widgets to overlap this one.
              // Require previous frame HoveredId to match before being usable.
              // Shortcut to calling SetNextItemAllowOverlap().
...
enum ImGuiPopupFlags_ {
  ...
  ImGuiPopupFlags_MouseButtonLeft =
      1 << 2, // For BeginPopupContext*(): open on Left Mouse release. Only one
              // button allowed!
  ImGuiPopupFlags_MouseButtonRight =
      2 << 2, // For BeginPopupContext*(): open on Right Mouse release. Only one
...
enum ImGuiSelectableFlags_ {
  ...
  ImGuiSelectableFlags_AllowOverlap =
      1 << 4, // Hit testing will allow subsequent widgets to overlap this one.
              // Require previous frame HoveredId to match before being usable.
              // Shortcut to calling SetNextItemAllowOverlap().
...
enum ImGuiKey : int {
  ...
  ImGuiKey_GamepadFaceDown,  // A           | B       | Cross    | Activate /
                            // Open / Toggle. Hold for 0.60f to Activate in Text
                            // Input mode (e.g. wired to an on-screen keyboard).
  ...
  ImGuiKey_GamepadDpadDown, // D-pad Down  | "       | "        | Move / Tweak /
                            // Resize Window (in Windowing mode)
  ImGuiKey_GamepadL1, // L Bumper    | L       | L1       | Tweak Slower / Focus
                      // Previous (in Windowing mode)
  ImGuiKey_GamepadR1, // R Bumper    | R       | R1       | Tweak Faster / Focus
...
enum ImGuiConfigFlags_ {
  ...
  // [Unused] User storage (to allow your backend/engine to communicate to code
  // that may be shared between multiple projects. Those flags are NOT used by
  // core Dear ImGui)
...
enum ImGuiButtonFlags_ {
  ...
  ImGuiButtonFlags_AllowOverlap =
      1 << 12, // Hit testing will allow subsequent widgets to overlap this one.
               // Require previous frame HoveredId to match before being usable.
               // Shortcut to calling SetNextItemAllowOverlap().
};
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_NoDragDrop =
      1 << 9, //              // ColorEdit: disable drag and drop target/source.
...
enum ImGuiSliderFlags_ {
  ...
  ImGuiSliderFlags_InvalidMask_ =
      0x7000000F, // [Internal] We treat using those bits as being potentially a
                  // 'float power' argument from legacy API (obsoleted 2020-08)
                  // that has got miscast to this enum, and will trigger an
                  // assert if needed.
};
...
enum ImGuiTableFlags_ {
  ...
  ImGuiTableFlags_Reorderable =
      1 << 1, // Enable reordering columns in header row. (Need calling
              ...
              // using ImGuiTableFlags_ContextMenuInBody to access context-menu
              // without headers).
...
struct ImGuiStyle {
  ...
  IMGUI_API ImGuiStyle();
  IMGUI_API void
  ScaleAllSizes(float scale_factor); // Scale all spacing/padding/thickness
...
struct ImGuiIO {
  ...
  ImVec2 DisplayFramebufferScale; // = (1, 1)         // Main display density.
  ...
  bool ConfigInputTextEnterKeepActive; // = false          // [BETA] Pressing
                                       // Enter will reactivate item and select
  ...
  bool
      ConfigDebugHighlightIdConflicts; // = true           // Highlight and show
...
struct ImGuiListClipper {
  ...
  // memset((void*)this, 0, sizeof(*this)); ItemsCount = -1; Begin(items_count,
...
enum ImGuiMultiSelectFlags_ {
  ...
  ImGuiMultiSelectFlags_SelectOnClickAlways =
      1 << 14, // Apply selection on mouse down when clicking on any items.
...
enum ImDrawFlags_ {
  ...
  ImDrawFlags_RoundCornersTopLeft =
      1 << 4, // Round top-left corner only (when rounding > 0.0f, we default to
              // all corners).
  ImDrawFlags_RoundCornersTopRight =
      1 << 5, // Round top-right corner only (when rounding > 0.0f, we default
              // to all corners).
  ImDrawFlags_RoundCornersBottomLeft =
      1 << 6, // Round bottom-left corner only (when rounding > 0.0f, we default
              // to all corners).
...
struct ImDrawData {
  bool Valid;        // Only valid after Render() is called and before the next
                     // NewFrame() is called.
  int CmdListsCount; // == CmdLists.Size. (OBSOLETE: exists for legacy reasons).
...
enum ImTextureStatus {
  ...
  ImTextureStatus_WantUpdates, // Requesting backend to update specific blocks
                               ...
                               // when done.
  ImTextureStatus_WantDestroy, // Requesting backend to destroy the texture. Set
                               // status to Destroyed when done.
};
...
struct ImTextureData {
  ...
  unsigned char *Pixels; // w    r   // Pointer to buffer holding 'Width*Height'
                         // pixels and 'Width*Height*BytesPerPixels' bytes.
  ...
  unsigned short RefCount; // w    r   // Number of contexts using this texture.
                           // Used during backend shutdown.
  ...
  // Functions
  // - If GetPixels() functions asserts while being called by your render loop,
  // it could be caused by calling ImFontAtlas::Clear()/ClearFonts()?
  ImTextureData() {
    memset((void *)this, 0, sizeof(*this));
...
struct ImFontConfig {
  ...
  float SizePixels; //          // Output size in pixels for rasterizer (more or
  ...
  unsigned int FontLoaderFlags; // 0        // Settings for custom font builder.
  ...
  float
      RasterizerDensity; // 1.0f     // [LEGACY: this only makes sense when
                         ...
                         // supported] DPI scale multiplier for rasterization.
                         // Not altering other font metrics: makes it easy to
                         // swap between e.g. a 100% and a 400% fonts for a
                         // zooming display, or handle Retina screen. IMPORTANT:
                         // If you change this it is expected that you
                         // increase/decrease font scale roughly to the inverse
...
struct ImFontAtlas {
  ...
  // Clearing the atlas/fonts has little use nowadays, unless you want to batch
  // remove all fonts.
  // - Since 1.92, you can call ClearFonts() mid-frame, if you load new fonts
  // afterwards.
  // - As we are transitioning toward our new font system the semantic for those
  // functions gets increasingly misleading and are often a source of issues.
  ...
  IMGUI_API void ClearFonts(); // Clear input+output font data/glyphs. New fonts
  ...
  IMGUI_API void
  RemoveCustomRect(ImFontAtlasRectId
                       id); // Unregister a rectangle. Existing pixels will stay
                            // in texture until resized / garbage collected.
  ...
  ImTextureFormat TexDesiredFormat; // Desired texture format (default to
                                    // ImTextureFormat_RGBA32 but may be changed
  ...
  bool Locked; // Marked as locked during ImGui::NewFrame()..EndFrame() scope if
               // TexUpdates are not supported. Any attempt to modify the atlas
               // will assert.
  ...
  bool TexPixelsUseColors; // Tell whether our texture data is known to use
                           ...
                           // process.
  ImVec2 TexUvScale; // = (1.0f/TexData->TexWidth, 1.0f/TexData->TexHeight). May
                     // change as new texture gets created.
  ImVec2 TexUvWhitePixel; // Texture coordinates to a white pixel. May change as
  ...
  unsigned int
      FontLoaderFlags; // Shared flags (for all fonts) for font loader. THIS IS
                       // BUILD IMPLEMENTATION DEPENDENT (e.g. Per-font override
...
struct ImFontBaked {
  ...
  unsigned int
      LoadNoRenderOnLayout : 1; // 0  //     // Enable a two-steps mode where
                                ...
                                // advantageous if you know that the glyph is
                                // unlikely to actually be rendered, otherwise
                                // it is slower because we'd do one query on the
                                // first CalcTextSize and one query on the first
                                // Draw.
...
enum ImFontFlags_ {
  ...
  ImFontFlags_NoLoadError =
      1 << 1, // Disable throwing an error/assert when calling AddFontXXX() with
              // missing file/data. Calling code is expected to check
  ...
  ImFontFlags_LockBakedSizes =
      1 << 3, // [Internal] Disable loading new baked sizes, disable garbage
              ...
              // single size. Important: if you use this to preload given sizes,
              // consider the possibility of multiple font density used on
              // Retina display.
...
struct ImFont {
  ...
  ImWchar EllipsisChar; // 2-4   // out // Character used for ellipsis rendering
                        // ('...'). If you ever want to temporarily swap this
                        // for an alternative/dummy char, make sure to clear
                        // EllipsisAutoBake.
...
struct ImGuiViewport {
  ...
  ImVec2 FramebufferScale; // Density of the viewport for Retina display (always
...
struct ImGuiPlatformIO {
  ...
  ImDrawCallback
      DrawCallback_SetSamplerNearest; // Request backend to set texture sampling
                                      // to Nearest/Point.
  // ImDrawCallback  DrawCallback_SetSamplerCustom;    // Request backend to set
  // texture sampling using Backend Specific data.
  ...
  ImVector<ImTextureData *>
      Textures; // List of textures used by Dear ImGui (most often 1) + contents
                // of external texture list is automatically appended into this.
  ...
  IMGUI_API void
  ClearPlatformHandlers(); // Clear all Platform_XXX fields. Typically called on
                           // Platform Backend shutdown.
  IMGUI_API void
  ClearRendererHandlers(); // Clear all Renderer_XXX fields. Typically called on
...
#define ImDrawCallback_ResetRenderState                                        \
  (ImDrawCallback)(-8) // OBSOLETED in 1.92.8: Use
