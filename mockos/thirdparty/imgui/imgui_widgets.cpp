    ...
    if (mods_ok) {
      if (mouse_button_clicked != -1 && g.ActiveId != id) {
        ...
        if (flags & ImGuiButtonFlags_PressedOnRelease) {
          // FIXME: Traditionally ImGuiButtonFlags_PressedOnRelease never took
          // ActiveId. Adding it in 2026-03-20 since
          // ImGuiButtonFlags_NoHoldingActiveId can always be added. We don't
          // yet perform an explicit ClearActiveID() to reduce scope of change,
          // but this possibility could be investigated.
          if (!(flags & ImGuiButtonFlags_NoHoldingActiveId))
            SetActiveID(id, window); // Hold on ID
          g.ActiveIdMouseButton = (ImS8)mouse_button_clicked;
        }
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg,
  ...
  RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding,
                    label, label_end, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::DataTypeApplyFromText(const char *buf, ImGuiDataType data_type,
  ...
  if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double) {
    ...
    if (format[0] == '\0')
      format =
          type_info
              ->ScanFmt; // Format doesn't want us to show the number currently,
                         // but we still need to parse the resulting input
  }
...
bool ImGui::TempInputText(const ImRect &bb, ImGuiID id, const char *label,
  ...
  bool value_changed = InputTextEx(
      label, NULL, buf, (int)buf_size, bb.GetSize(),
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type,
  ...
  bool input_edited =
      ...
      0; // We would be using 'ret' if ImGuiInputTextFlags_EnterReturnsTrue was
         // not involved.
  bool value_changed =
      input_edited
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  if (g.ActiveId == id) {
    ...
    if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
      ...
      if (event_flag) {
        ...
        if (callback_data.BufDirty) {
          // Callback may update buffer and thus set buf_dirty even in read-only
          // mode.
  ...
  // for details) This is used when e.g. losing focus or tabbing out into
  // another InputText() which may already be using the temp buffer.
  ...
  // - Make sure we always reapply the live buffer back to the input/user buffer
  // before clearing ActiveId, even thought strictly speaking
  ...
  //    allocated/string object every frame. Which in the grand scheme of scheme
  //    is nothing, but isn't dear imgui vibe).
...
bool ImGui::BeginBoxSelect(const ImRect &scope_rect, ImGuiWindow *window,
  ...
  // selection. They are derived from positions relative to scrolling space, so
  // "previous" rectangle is reprojected for current frame coordinates.
  ...
  // Storing unclip rects which will be tested by widgets supporting box-select.
  // Always update rectangles when active (even if we don't use them). To
  // facilitate understanding this: enable IMGUI_DEBUG_BOXSELECT and visualize
  // all geometry.
  if (ms_flags &
    ...
    // UnclipRects[0]/[1] Areas, widgets should ideally first use UnclipRect as
    // a first coarse cull layer + the individual ones as a second validation.
    ...
    if (bs->UnclipMode && g.CurrentTable != NULL)
      TableApplyExternalUnclipRect(g.CurrentTable,
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData *ms, ImGuiWindow *window) {
  if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect) {
    ...
    // EndMultiSelect() only This probably doesn't work inside a table as there
    // are ample ambiguities related to exact time of calling
...
ImGuiMultiSelectIO *ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags,
  ...
  if (flags & ImGuiMultiSelectFlags_ScopeRect)
    window->DC.CursorMaxPos =
        ms->ScopeRectMin; // CalcScopeRect() for ImGuiMultiSelectFlags_ScopeRect
                          // will measure in EndMultiSelect().
...
void ImGui::SetNextItemSelectionUserData(
  ...
  if (ImGuiMultiSelectTempData *ms = g.CurrentMultiSelect) {
    ...
    // g.NextItemData.SelectionUserData because NextItemData is not restored on
    // nested multi-select. ms->CurrSubmittedItem = selection_user_data;
  } else {
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool *p_selected,
  ...
  if (p_button_flags != NULL) {
    ...
    if (ms->Flags & ImGuiMultiSelectFlags_SelectOnClickAlways)
      ...
      button_flags |=
          (!selected || (g.ActiveId == id && g.ActiveIdHasBeenPressedBefore))
              ? ImGuiButtonFlags_PressedOnClick
              : ImGuiButtonFlags_PressedOnClickRelease;
      ...
      if (!window->DC.NavIsScrollPushableX) // FIXME: Rename to be more generic.
        if (ImGuiTable *table = g.CurrentTable)
          if (table->CurrentColumn != -1) {
            // FIXME: We cannot solely use current ClipRect as it includes
            // HostClipRect. However we account for ClipRect being larger than
            // current column (e.g. when using SpanAllColumns) A more generic
            // version would be nice, but window->WorkRect.Min/Max exclude
            ...
            if (clip_min_x !=
                clip_max_x) // When zero sized we expect that bounds have been
                            // clamped and thus are unreliable
            {
              item_rect.Min.x =
      ...
      if ((rect_overlap_curr && !rect_overlap_prev && !selected) ||
        ...
        if (storage->LastSelectionSize <= 0 && bs->IsStartedSetNavIdOnce) {
          ...
          GetForegroundDrawList()->AddRectFilled(
              ...
              selected ? IM_COL32(0, 255, 0, 200) : IM_COL32(255, 0, 0, 200));
#endif
        }
...
void ImGui::MultiSelectAddSetRange(ImGuiMultiSelectTempData *ms, bool selected,
  ...
  // value). The low-hanging fruit would be to know that ImGuiSelectionUserData
  // are sequential indices, in which case we can trivially compare
  // PrevSubmittedItem + RangeDir == FirstItem. User can always perform this
  // merge if required.
#if 0
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled) {
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    float min_w = offsets->DeclColumns(icon_w, label_size.x, 0.0f,
  ...
  // Once dragged, release ActiveId + key ownership. This is to allow the idiom
  // of mouse down a menu, dragging elsewhere, up on some other MenuItem().
  // (#8233, #9394) Could move logic into lower-level
...
bool ImGui::MenuItemEx(const char *label, const char *icon,
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    PushStyleVarX(ImGuiStyleVar_ItemSpacing, style.ItemSpacing.x * 2.0f);
    pressed =
        Selectable("", selected, selectable_flags, ImVec2(label_size.x, 0.0f));
    PopStyleVar();
    ...
    float checkmark_w = IM_TRUNC(g.FontSize * 1.20f);
    float min_w = offsets->DeclColumns(icon_w, label_size.x, shortcut_w,
