...
//  2026-06-23: OpenGL: GLSL version detection assume GLSL 410 when GL context
//  is 4.1. Fixes an issue running on macOS with Wine. (#9427, #6577)
...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData *draw_data,
  ...
  if (bd->HasBindSampler)
    glBindSampler(0,
                  bd->TexSamplers[0]); // We use combined texture/sampler state.
...
static void
ImGui_ImplOpenGL3_DrawCallback_SetSamplerNearest(const ImDrawList *,
                                                 const ImDrawCmd *) {
  ImGui_ImplOpenGL3_Data *bd = ImGui_ImplOpenGL3_GetBackendData();
  if (bd->HasBindSampler) {
  ...
  for (const ImDrawList *draw_list : draw_data->CmdLists) {
    ...
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        // Emulate sampler change (even though it is technically part of texture
        // data) As a sort of hack/workaround, we only start writing using
        // glTextParameter() if sampler is ever changed explicitly.
  ...
  if (bd->HasBindSampler) {
    ...
    for (int sampler_n = 0; sampler_n < 2; sampler_n++) {
      ...
      GL_CALL(glSamplerParameteri(bd->TexSamplers[sampler_n],
                                  ...
                                  (sampler_n == 0) ? GL_LINEAR : GL_NEAREST));
      GL_CALL(glSamplerParameteri(bd->TexSamplers[sampler_n], GL_TEXTURE_WRAP_S,
                                  GL_CLAMP_TO_EDGE));
      GL_CALL(glSamplerParameteri(bd->TexSamplers[sampler_n], GL_TEXTURE_WRAP_T,
                                  GL_CLAMP_TO_EDGE));
    }
  }
#endif
