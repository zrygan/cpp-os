...
inline void MakeTextEditor(mockos::Context *this_ctx) {
  if (ImGui::Begin("Notepad.exe", &this_ctx->flags.show_text_editor,
    ...
    if (ImGui::BeginMenuBar()) {
      if (ImGui::BeginMenu("File")) {
        ...
        if (ImGui::MenuItem("Exit")) {
          this_ctx->flags.show_text_editor = false;
        }
        ImGui::EndMenu();
      }
      ImGui::EndMenuBar();
    ...
    ImGui::InputTextMultiline("##TextEditorArea", text_buffer,
                              ...
                              ImGui::GetContentRegionAvail(), kTextEditorFlags);
  }
  ImGui::End();
}
